...
function buildRow(signal) {
  return [
    ...
    signal.enrichment?.funding_signal || 'none',
    signal.enrichment?.funding_round_guess || 'unknown',
    signal.enrichment?.funding_amount_guess ?? '',
    signal.enrichment?.funding_currency_guess || '',
    signal.enrichment?.investor_signal || 'none',
    signal.enrichment?.lead_investor_guess || '',
    (signal.enrichment?.investor_list_guess || []).join('|'),
...
class CrmExportWriter {
  ...
  write(signals) {
    ...
    const header = [
      ...
      'funding_signal',
      'funding_round_guess',
      'funding_amount_guess',
      'funding_currency_guess',
      'investor_signal',
      'lead_investor_guess',
      'investor_list_guess',
      'hiring_signal',
